...
public class EnemySpawner : MonoBehaviour
{
 ...
 private void Awake()
  ...
  public bool GetRandPointInArea( out Vector3 result)
  {
      ...
      var randomPoint =  new Vector3(Random.Range(Area.GetComponent<Collider2D>().bounds.min.x ,Area.GetComponent<Collider2D>().bounds.max.x),Random.Range(Area.GetComponent<Collider2D>().bounds.min.y ,Area.GetComponent<Collider2D>().bounds.max.y) , 0) ;
  ...
  private IEnumerator Spawn()
  {
      ...
      while (CurNumberofSpawns < SpawnAmount)
      {
          ...
          {
              ObjectPoolManager.SpawnObject( EnemyToSpawn,result,Quaternion.identity,ObjectPoolManager.PoolType.Enemies );
              CurNumberofSpawns++;
